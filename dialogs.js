...
window.descriptions = {
  packagedevel: "<ul><li><strong>Type:</strong> Starting Point. Action</li><li><strong>Role:</strong> Package Maintainer</li></ul><p>A package is usually developed in a Home Project. We can consider it as the starting poing for this workflow, together with the Devel Project Integration task.</p><p>With OBS, a contributor can make a local branch (usually from a Devel Project) of the package to start working on it. Once the developer finishes with the package, he creates a Submit Request to send it back.</p>This is usually how packages are being developed: They start in home project, move to devel project and continue to Factory.</p>",

  createsr: "<ul><li><strong>Type:</strong>Action</li><li><strong>Role:</strong> Package Maintainer</li></ul><p>A Submit Request is a request of integration from one project to another. This object also captures all the changes made in the package, with some of the meta information related with this petition (history of approvals, current status, etc).</p><p>If the developer wants to include the updated version of the package, she / he needs to create a formal petition via this SR. We also have policies that describe what extra actions need to be done when the developer wants to add a new package to Factory.</p>",

  acceptsr: "<ul><li><strong>Type:</strong> Condition</li><li><strong>Role:</strong> Project Maintainer</li></ul><p>The Project Maintainer can decide whether to accept or reject the SR according to rules described in external policies.</p><p>This decision is expressed via OBS and usually the rejection is justified in order to give the submitter some feedback and a chance to submit changes again.</p>",

  develprojectint: "<ul><li><strong>Type:</strong> Starting Point. Action</li><li><strong>Role:</strong> Project Maintainer</li></ul><p>The integration can be defined as the process of merging all packages together in a single tree, creating a consistent whole inside the Devel Project.</p><p>This integration can have two different origins: <ol><li>When the Project Maintainer is working in his own project (in this case this action can be considered as a starting point)</li><li>As a result of the integration of the SR generated by an external contributor.</li></ol></p><p>The Package Maintainer can also work on her / his own package without the Project Maintainer intervention.</p>",
  ...
  rpmlint: "<ul><li><strong>Type:</strong> Condition</li> \
	  ...
	  that there are no overrides of rpmlint rules that bypass policies</p>",
  ...
  integrationtest: "<ul><li><strong>Type:</strong> Condition</li><li><strong>Role:</strong> QA Team</li></ul><p>Integration tests are done by QA Team when a new SR is integrated in Factory and needs to be tested before passing to Factory-Tested. When the Factory Maintainer integrates a new SR, there is a good possibility that some part of the system stops working. The Integration Test is designed to assure that the integration of a new package doesn't affect the functionality of a different package in a unnoticed way.</p><p>To provide this, QA Team will run a set of transversal tests along the full distribution, to make sure that the system works similar to how it worked before integrating the SR.</p><p>This task also decides if the new SR that comes from Factory goes to the next step until Factory-Tested. If QA Team detect an important issue related with the new SR, they can decline the copy of the new package (or group of packages) to Factory-Tested.</p>",
  ...
  legalreviewer: "<ul><li><strong>Type:</strong> Role</li><li><strong>Current role:</strong> Legal team</li></ul><p><ul><li>Checks whether we can legally distribute a package.</li><li>Reviews whether the declared license is correct and looks for possible conflicting licenses.</li><li>Checks license changes among packages.</li></ul></p>",
  ...
  technicalreviewer: "<ul><li><strong>Type:</strong> Role</li><li><strong>Role:</strong>Technical Reviewer</li><li><strong>Current role:</strong> Technical Reviewer</li></ul><p>Reviews submissions to Factory to make sure they follow packaging guidelines and don't contain obvious errors.</p><p>Newly technical reviewer also groups packages related to each other (think new KDE) that are going to get accepted all together.</p><p>He/she also decides, when the change might have a big enough impact to run it through staging project first (new autotools, boost, gcc, icu, ...) or when the change is big and might seriously break things for people in runtime so it needs to pass through openQA (think new Xorg, new systemd, ...)",
