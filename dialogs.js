...
window.descriptions = {
  ...
  develprojectint: "<ul><li><strong>Type:</strong> Starting Point. Action</li><li><strong>Role:</strong> Project Maintainer</li></ul><p>The integration can be defined as the process of merging all packages together in a single tree, creating a consistent whole inside the Devel Project.</p><p>This integration can have two different origins: (1) one made directly by the Project Maintainer working in his own project (in this case this action can be considered as a starting point, and (2) another as a result of the integration of the SR generated by an external contributor.</p><p>The Package Maintainer also can work on her / his own package without the Project Maintainer intervention.</p>",
  ...
  stagingprojectint: "<ul><li><strong>Type:</strong> Action</li><li><strong>Role:</strong> Project Maintainer</li></ul><p>Staging Projects is one of the most important changes proposed for the new workflow. And staging project is assigned by the Technical Reviewer for packages that can affect multiple other packages. For example: GCC, Autotools, systemd or maybe NetworkManager.</p><p>T</p>

How are requests being managed in staging project.</p><p>They start as a sumbission to factory that is deemed to require new staging project creation or that they are part of already existing request.</p><p>They are then being developed on specific branch of openSUSE:Factory where the designated maintainer with package maintainers fix the issues to have everything working just fine.</p><p>Whenever the developers see fit they can sent the staging changes back to devel projects.</p><p>When the packages are seemingly fine the staging project gets dismantled and all requests in group get forwarded further on in review cycle for Factory submissions.</p>",
